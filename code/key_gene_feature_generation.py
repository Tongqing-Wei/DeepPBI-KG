...
def protein_process(path, key_gene):
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--input_phage_key_gene', type=str, help = 'input phage key gene list csv file path')
    ...
    parser.add_argument('--input_host_key_gene', type=str, help = 'input host key gene list csv file path')
    parser.add_argument('--output', type=str, help = 'output phage and host dna protein feature')
    opt = parser.parse_args()   
    
    
    # phage feature process
    phage_dna = dna_process(opt.input_phage_cds_dna, opt.input_phage_key_gene)
    phage_protein = protein_process(opt.input_phage_cds_protein, opt.input_phage_key_gene)
    ...
    phage.iloc[:,index].to_csv(opt.output + os.sep + 'phage_key_gene_feature.csv', index = True)
    
    
    # host feature process
    host_dna = dna_process(opt.input_host_cds_dna, opt.input_host_key_gene)
    host_protein = protein_process(opt.input_host_cds_protein, opt.input_host_key_gene)
    ...
    for i in range(6):
        index.extend([89 + i * 90 + 798] + list(range(0 + i * 90 + 798, 89 + i * 90 + 798)))
    host.iloc[:,index].to_csv(opt.output + os.sep + 'host_key_gene_feature.csv', index = True)