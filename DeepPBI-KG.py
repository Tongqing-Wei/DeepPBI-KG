...
def get_align_and_interaction_infor(phage_align, phage_raw_data, host_align, host_raw_data, inter, result, pa_fasta_path, ha_fasta_path,
    ...
    os.mkdir(pa_fasta_path)
    os.mkdir(ha_fasta_path)
    os.mkdir(pi_infor_path)
    os.mkdir(hi_infor_path)
    ...
    dic_p_rd = {}
    dic_h_rd = {}
    for i in range(len(phage_raw_data)):
        dic_p_rd[phage_raw_data[i].split('/')[-1][:-6]] = phage_raw_data[i]
    for i in range(len(host_raw_data)):
        dic_h_rd[host_raw_data[i].split('/')[-1][:-6]] = host_raw_data[i]
    ...
    for i in range(len(phage_align)):
        if os.stat(phage_align[i]).st_size == 0:
            ...
            f.close()
            sp = line[0].split('\t')
            dic_p[phage_align[i].split('/')[-1][:-4]] = [sp[1].split('.')[0], sp[-2]]
            shutil.copyfile(dic_p_rd[sp[1].split('.')[0]], pa_fasta_path + os.sep + sp[1].split('.')[0] + '.fasta')
            inter[inter['phage'] == sp[1].split('.')[0]].iloc[:, [1,3]].to_csv(pi_infor_path + os.sep + sp[1].split('.')[0] + '.log', index = False)
    ...
    for i in range(len(host_align)):
        if os.stat(host_align[i]).st_size == 0:
            ...
            f.close()
            st = line[0].split('\t')
            dic_h[host_align[i].split('/')[-1][:-4]] = [st[1].split('.')[0], st[-2]]    
            shutil.copyfile(dic_h_rd[st[1].split('.')[0]], ha_fasta_path + os.sep + st[1].split('.')[0] + '.fasta')
            inter[inter['host'] == st[1].split('.')[0]].iloc[:, [0,2]].to_csv(hi_infor_path + os.sep + st[1].split('.')[0] + '.log', index = False)
...
def set_Hello():
    ...
    hello_div.add(b(font('Hi Users,')))
    hello_div.add(b(font('this is the prediction result of DeepPBI-KG')))
...
def fill_table_data(data_type, pp, hh, kg, wg, plink_text, plink_url, hlink_text, hlink_url, pilink_text, pilink_url, hilink_text, hilink_url):
    ...
    data_tr += link_td1
    link_td2 = td()
    link_td2 += a(hlink_text, href=hlink_url)
    data_tr += link_td2
    link_td3 = td()
    link_td3 += a(pilink_text, href=pilink_url)
    data_tr += link_td3
    link_td4 = td()
    link_td4 += a(hilink_text, href=hilink_url)
    data_tr += link_td4

def generate_result_table(res):
...
def generate_ending():
    br()
    p('** Feel free to connect 20210700125@fudan.edu.cn if you have any question. **')
